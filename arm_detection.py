...
def crop_below_point(image, start_x, start_y, crop_width, crop_height):
    h, w = image.shape[:2]
...
def detect_armrest_and_annotate(image, landmarks, base_name, isDesk=False, isChair=True):
    ...
    def detect_in_roi(center_x, center_y, crop_w, crop_h):
        ...
        cropped_edges, crop_x1, crop_y1 = crop_below_point(edges, center_x, center_y, crop_w, crop_h)
        ...
        if lines is not None:
            for x1, y1, x2, y2 in lines[:, 0]:
                ...
                if -0.5 < slope < 0.5:  # horizontal-ish
                    ...
                    candidates.append({
                        ...
                        "w": x2 - x1,
                        "h": 20,
                        "score": length  # can add more scoring metrics
                    })
        return candidates
    ...
    if isChair:    
        # Search below elbow
        below_candidates = detect_in_roi(elbow["x"], elbow["y"], 300, 200)

        # Search above elbow (from elbow up towards shoulder)
        above_roi_height = max(50, abs(shoulder["y"] - elbow["y"]))
